      ...
      footer={
        selectedActivity ? (
          <button
            ...
            className="w-full py-3.5 bg-emerald-600 text-white font-semibold rounded-2xl shadow-sm active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {submitting ? (
              <>
    ...
    >
      {loading ? (
        <div className="flex flex-col items-center justify-center py-16">
          <div className="w-12 h-12 border border-stone-200 border-t-transparent rounded-full animate-spin mb-3" />
          <p className="text-stone-600 font-medium">Katalog yuklanmoqda...</p>
        </div>
      ) : !selectedActivity ? (
        <div className="pb-4">
          {/* Search */}
          <div className="sticky top-0 z-10 bg-white px-5 pt-3 pb-2">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 bg-stone-50 rounded-2xl border border-transparent focus:border-stone-400 focus:bg-white outline-none transition-all font-medium text-stone-900"
              />
            </div>
            ...
            <div className="flex gap-2 overflow-x-auto scrollbar-hide mt-3">
              <button
                ...
                className={`px-4 py-2 rounded-full whitespace-nowrap text-sm font-semibold transition-all ${selectedCategory === "all"
              ...
              {categories.map((cat) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full whitespace-nowrap text-sm font-semibold transition-all flex items-center gap-1.5 ${selectedCategory === cat.id
                      ? "bg-emerald-600 text-white shadow-sm"
                      : "bg-stone-50 text-stone-600 hover:bg-stone-100"
                    }`}
                >
            ...
            {filteredActivities.length === 0 ? (
              ...
              filteredActivities.map((activity) => (
                <button
                  ...
                  className="w-full flex items-center gap-3 p-3 bg-white border border-stone-200 rounded-2xl hover:border-stone-200 hover:bg-stone-50 active:scale-[0.98] transition-all text-left"
                >
                  <span className="text-2xl bg-white w-12 h-12 flex items-center justify-center rounded-xl flex-shrink-0">
                    {activity.icon}
                  </span>
        ...
        <div className="p-5 space-y-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="w-12 h-12 rounded-2xl bg-stone-50 hover:bg-stone-100 active:scale-90 flex items-center justify-center text-xl font-semibold transition-all"
              >
                −
              </button>
              <div className="flex-1 relative">
                <input
                  ...
                  className="w-full text-center text-2xl font-semibold py-3 border border-stone-200 bg-white rounded-2xl focus:border-stone-400 focus:bg-white outline-none transition-all"
                />
                <span className="absolute -bottom-5 left-0 right-0 text-center text-xs text-stone-500 font-medium">
                  daqiqa
                </span>
              </div>
              <button
            ...
            <div className="flex justify-center gap-2 mt-7">
              {[15, 30, 45, 60, 90].map((min) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-sm font-semibold transition-all ${duration === min
                      ? "bg-emerald-600 text-white shadow-sm"
                      : "bg-stone-50 text-stone-600 hover:bg-stone-100"
                    }`}
                >
          ...
          {selectedActivity.has_distance && (
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-stone-50 border border-transparent rounded-2xl focus:border-stone-400 focus:bg-white outline-none font-semibold transition-all"
              />
            </div>
          )}
          ...
          <div className="bg-amber-50 p-4 rounded-2xl border border-amber-200 text-center">
            ...
            <div className="text-4xl font-semibold text-amber-700 flex items-center justify-center gap-2">
              <span>🔥</span>
              <span>{calculateCalories()}</span>
              <span className="text-base text-amber-700 font-semibold">kkal</span>
            </div>
            {!user?.weight_kg && (
